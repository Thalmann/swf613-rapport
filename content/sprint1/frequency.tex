...
It is still unclear how and why exactly the frequency spectrum is used for extracting a single frequency, it is only certain that the result from the FFT and the following manipulation leads to a single frequency value.
This single value is then used to compare with the calibrated minimum and maximum values, to determine in which direction to move the car.
...
This means that lower frequencies that are very different in regards to tone, may be grouped together.

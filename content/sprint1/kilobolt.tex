...
Besides the canvas, the render-thread also uses a bitmap buffer which the canvas uses for drawing.\bruno{double buffering}
...
As the rendering of frames is done continuously in a separate thread, there is no way to ensure a fixed framerate.\bruno{thomas: frame independent movement}
